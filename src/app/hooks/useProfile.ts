...
function mapToUserProfile(raw: any): UserProfile {
  ...
  return {
    ...
    tipoDocumento: String(raw?.cliTipoDocumento ?? raw?.cli_documento_tipo ?? raw?.tipoDocumento ?? 'DNI'),
    documento:     String(raw?.cliDocumento ?? raw?.cli_documento ?? raw?.documento ?? ''),
